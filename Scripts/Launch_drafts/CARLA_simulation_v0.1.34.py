...
Versioning: v0.1.34 (Reverted to User's v0.1.30)
...
class HUD(object):
    def __init__(self, width, height):
        ...
        sound_files = {
            "collision": "collision_alert.wav", # User needs to provide this
            "lane_violation": "lane_deviation.wav", # User needs to provide this
            "speeding": "SpeedLimit.wav", # User needs to provide this
            "error": "ErrorEncountered.wav", # User needs to provide this
            "default_notification": "alert_sound.wav" # User needs to provide this
        }
        ...
        # MODIFIED: Stacked notifications start even higher (35% from top)
        self._notification_base_pos_y = int(self.dim[1] * 0.35) 
    ...
    def reset_warning_trackers(self):
        ...
        # MODIFIED: Reset all sound timestamps
        self._last_sound_time = {sound_type: 0.0 for sound_type in self.sound_cooldowns}
    ...
    # MODIFIED: New helper method to play sounds with cooldown
    def play_sound_for_event(self, event_type, force_play=False):
        sound_to_play = self.sounds.get(event_type)
        if not sound_to_play:
            sound_to_play = self.sounds.get("default_notification") # Fallback to default
            if not sound_to_play:
                return # No sound available at all

        current_time = time.time()
        cooldown = self.sound_cooldowns.get(event_type, 0.0) # Get specific cooldown, default 0
    ...
    def deduct_score(self, points, violation_type="unknown"):
        ...
        penalty_message = ""
        sound_event_type = violation_type # Assume violation_type is a key in sounds/cooldowns
        ...
        if violation_type == "collision":
            ...
            sound_event_type = "default_notification" # For unknown types, use default sound
        ...
        self.play_sound_for_event(sound_event_type)
    ...
    def tick(self, world, clock):
        ...
        if speed_kmh > 120: 
            if self.frame > self._last_speed_warning_frame_warned + 180: 
                ...
                self.play_sound_for_event("speeding") # MODIFIED: Use helper for speeding sound
    ...
    def critical_alert(self, text, seconds=2.0, text_color=(255, 255, 255), symbol_color=(255, 255, 0)):
        self.notification(text, seconds=seconds, text_color=text_color, 
                          ...
                          is_blinking=True, is_critical_center=True)
        self.play_sound_for_event("error", force_play=True) # MODIFIED: Use helper, force play for critical
    ...
    def error(self, text):
        self.notification('ERROR: %s' % text.upper(), seconds=5.0, text_color=(255,50,50), 
                          symbol_enabled=True, symbol_color=(255,0,0), 
                          is_blinking=True, is_critical_center=True)
        self.play_sound_for_event("error", force_play=True) # MODIFIED: Use helper, force play for error
...
class BlinkingAlert(object):
    ...
    def set_text(self, text, text_color=(255, 255, 255), seconds=2.0, 
        ...
        base_rgb = (0, 0, 0) # MODIFIED: Black base for gradient
...
class LaneInvasionSensor(object):
    ...
    def _on_invasion(weak_self, event):
        ...
        if player_forward_vec_2d.length_squared() > 0: # Avoid division by zero if vector is (0,0)
        ...
        if occupied_lane_forward_vec_2d.length_squared() > 0:
        ...
        if dot_product_with_occupied_lane < -0.7: # Threshold for "oncoming"
            ...
            for marking in event.crossed_lane_markings:
                if marking.type in [carla.LaneMarkingType.Solid, 
                                    ...
                                    carla.LaneMarkingType.DoubleSolidYellow,
                                    carla.LaneMarkingType.SolidYellow,
                                    carla.LaneMarkingType.SolidWhite]:
