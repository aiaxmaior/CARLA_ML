...
class World(object):
    def __init__(self, client_obj, carla_world, hud, actor_filter, fov): # MODIFIED: Added fov argument
    ...
    def restart(self):
        ...
        self.spawn_traffic(num_vehicles=0, num_pedestrians=0) # Spawn traffic with default numbers
        
        actor_type = get_actor_display_name(self.player)
        self.hud.notification(f"{actor_type} Ready!")

    # NEW METHOD: Add this function to your World class
    def spawn_traffic(self, num_vehicles=0, num_pedestrians=0):
        ...
        # --- Spawn Vehicles ---
        # Filter for vehicle blueprints (excluding the player's current type to avoid duplicates if desired)
        vehicle_blueprints = [bp for bp in blueprint_library.filter('vehicle.*') if bp.id != self.player.type_id]
        
        # Get all spawn points from the map
        spawn_points = world.get_map().get_spawn_points()
        random.shuffle(spawn_points) # Shuffle to get varied spawn locations
        ...
        for i in range(len(spawn_points)):
            ...
            # Set vehicle attributes (e.g., color, low_quality for performance)
            if vehicle_bp.has_attribute('color'):
                color = random.choice(vehicle_bp.get_attribute('color').recommended_values)
                vehicle_bp.set_attribute('color', color)
            vehicle_bp.set_attribute('role_name', 'autopilot') # Assign a role name for traffic
            # vehicle_bp.set_attribute('texture_from_random_color', 'true') # Optional: random color

            # For performance, you can use 'low_quality' for distant vehicles
            # vehicle_bp.set_attribute('quality_level', 'Low') 

            vehicle = world.try_spawn_actor(vehicle_bp, spawn_point)
            if vehicle is not None:
                vehicle.set_autopilot(True) # Make it drive itself
                self.actors_to_destroy.append(vehicle) # Add to cleanup list
                vehicles_spawned += 1
        ...
        for i in range(num_pedestrians):
            ...
            # Get a random navigable location for pedestrians
            spawn_loc = world.get_random_location_from_navigation()
            if spawn_loc is None:
                print("Warning: Could not find a suitable pedestrian spawn location.")
                continue

            # Ensure spawn_loc is elevated slightly to avoid spawning inside ground
            spawn_loc.z += 2.0 
            spawn_point_ped = carla.Transform(spawn_loc)

            walker_bp = random.choice(walker_blueprints)
            # walker_bp.set_attribute('is_invincible', 'true') # Optional: make them not get hit
            # walker_bp.set_attribute('speed', '2.0') # Optional: set speed

            # Spawn the pedestrian actor
            pedestrian = world.try_spawn_actor(walker_bp, spawn_point_ped)
            if pedestrian is not None:
                # Spawn the walker AI controller
                walker_controller = world.try_spawn_actor(walker_controller_bp, carla.Transform(), attach_to=pedestrian)
                if walker_controller is not None:
                    # Start the walker AI controller
                    walker_controller.start()
                    # Make the walker move to a random point within the navigable area
                    walker_controller.go_to_location(world.get_random_location_from_navigation())
                    
                    self.actors_to_destroy.append(pedestrian)
                    self.actors_to_destroy.append(walker_controller) # Remember to destroy controller too
                    pedestrians_spawned += 1
                    # print(f"Spawned pedestrian {pedestrian.id} at {spawn_loc}")
                else:
                    pedestrian.destroy() # Destroy pedestrian if controller fails to spawn
                    print(f"Failed to spawn controller for pedestrian.")
            else:
                print(f"Failed to spawn pedestrian.")
                
        print(f"Successfully spawned {pedestrians_spawned} pedestrians.")

    # NEW METHOD: Add this cleanup function to your World class
    def destroy_traffic(self):
        print(f"Destroying {len(self.actors_to_destroy)} traffic actors...")
        client = self.client # Get the client instance
        client.apply_batch([carla.command.DestroyActor(x) for x in self.actors_to_destroy])
        self.actors_to_destroy.clear()
        print("Traffic actors destroyed.")
...
class DualControl(object):
    def __init__(self, world, start_in_autopilot):
        ...
        if joystick_count == 0:
            ...
            self._steer_joystick_idx = 0 
            self._pedal_joystick_idx = 0 
            
            self._steer_axis_idx = 0      
            self._throttle_axis_idx = 5   
            self._brake_axis_idx = 2      
            
            self._handbrake_button_idx = 0 
            self._reverse_button_idx = 2   
            self._gear_up_button_idx = 5   
            self._gear_down_button_idx = 4 
            self._gear_mode_manual = 1
            self._toggle_view = 3
            """
            ##################### FANATEC SETTINGS - SWITCH BACK TO THESE WHEN USING THE WHEEL ####################
    ...
    def parse_events(self, world, clock):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # if event.joy == self._steer_joystick_idx and event.button == 
...
def game_loop(args):
    ...
    try:
        ...
        world = World(client, client.get_world(), hud, args.filter, args.fov) 
