...
class HUD(object):
    def __init__(self, width, height):
        ...
        # MODIFIED: Stacked notifications start higher up (55% from top, which is 45% from bottom)
...
class BlinkingAlert(object):
    def __init__(self, font, screen_dim, initial_dim, symbol_font=None):
        ...
        self.outline_thickness = 2 # Changed from 4px to 2px for better clarity with gradient
    ...
    def set_text(self, text, text_color=(255, 255, 255), seconds=2.0, 
        ...
        # Total width includes content, padding on both sides, and the vertical bar + its own small padding
        ...
        base_rgb = (20, 20, 20) if self.is_critical_center else (30, 30, 30)
        # Area for gradient (left of the vertical bar)
        gradient_area_width = box_width - self.vertical_bar_width - (padding_horizontal // 3)
        
        alpha_left = int(255 * 0.1) # Target alpha on the far left
        alpha_right = 220           # Target alpha just before the vertical bar
        
        for x_col in range(gradient_area_width):
            # Ratio: 0 at left edge of gradient area, 1 at right edge of gradient area
        ...
        # --- Draw the vertical bar on the right (on top of the gradient's edge) ---
        bar_x = gradient_area_width 
        bar_rect = pygame.Rect(bar_x, 0, self.vertical_bar_width, box_height) # Bar covers full height
        pygame.draw.rect(self.surface, self.vertical_bar_color, bar_rect) # No border radius for the bar itself for sharp look

        # --- Text and Symbol Positioning (relative to the gradient area's padding) ---
        blit_x_start_content = padding_horizontal # Start content after left padding
        ...
        offsets = []
        # Create a more filled outline for thickness > 1
        for dx_outline in range(-self.outline_thickness, self.outline_thickness + 1):
            for dy_outline in range(-self.outline_thickness, self.outline_thickness + 1):
                # For a "square" or "diamond" outline shape, use:
                # if abs(dx_outline) + abs(dy_outline) <= self.outline_thickness and not (dx_outline == 0 and dy_outline == 0):
                # For a more "circular/full" outline, just check if it's not the center
                if not (dx_outline == 0 and dy_outline == 0):
                     offsets.append((dx_outline, dy_outline))
        
        if not offsets and self.outline_thickness > 0: # Fallback for 1px if above logic yields empty
