...
class DualControl(object):
    def __init__(self, world, start_in_autopilot):
        ...
        if joystick_count == 0:
            ...
            # --- IMPORTANT: THESE ARE PLAYSTATION CONTROLLER MAPPINGS:
            self._steer_joystick_idx = 1 # Often the first joystick detected
            self._pedal_joystick_idx = 1 # Assuming pedals are on the same joystick as wheel
            self._steer_axis_idx = 0 
            self._throttle_axis_idx = 4 # Check your controller mapping (e.g., R2 trigger)
            self._brake_axis_idx = 3    # Check your controller mapping (e.g., L2 trigger)
            self._handbrake_button_idx = 1 # Example: 'X' or 'A' button
            self._reverse_button_idx = 3   # Example: 'Triangle' or 'Y' button
            self._gear_up_button_idx = 5   # Example: R1 bumper
            self._gear_down_button_idx = 4 # Example: L1 bumper
            self._gear_mode_manual = 2     # Example: 'Square' or 'X' button (different from handbrake)
            # --- CHANGE BACK WHEN REVERTING TO FANATEC

            try:
                # Attempt to initialize the primary joystick (steering/pedals)
                # Ensure the index is valid for the detected joysticks
                if self._steer_joystick_idx < joystick_count:
                    ...
                    print(f"Warning: Steering joystick index {self._steer_joystick_idx} is out of range. Max index: {joystick_count-1}")
                    self._steer_joystick = None
                ...
                if self._pedal_joystick_idx != self._steer_joystick_idx:
                    if self._pedal_joystick_idx < joystick_count:
                        ...
                        print(f"Warning: Pedal joystick index {self._pedal_joystick_idx} is out of range. Max index: {joystick_count-1}")
                        self._pedal_joystick = None # Set to None if out of range
                else:
        ...
        self._steer_linearity = 0.32 # Adjust for desired steering sensitivity curve
    ...
    def parse_events(self, world, clock):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                elif event.key > K_0 and event.key <= K_9: # Corrected sensor indexing
                    if world.camera_manager: world.camera_manager.set_sensor(event.key - K_0 -1) # K_0 is 48, K_1 is 49. So K_1-K_0-1 = 0
                elif event.key == K_r:
                ...
                if isinstance(self._control, carla.VehicleControl) and world.player:
                    if event.key == K_q: # Q for reverse toggle (like many games)
                        current_control = world.player.get_control()
                        if current_control.gear == -1 : # If in reverse
                             self._control.gear = 1 # Go to Drive (or Neutral if speed is low)
                             self._control.reverse = False
                        else:
                            self._control.gear = -1 # Go to Reverse
                            self._control.reverse = True
                    elif event.key == K_m:
                        self._control.manual_gear_shift = not self._control.manual_gear_shift
                        current_player_control = world.player.get_control() # Get current gear from player
                        self._control.gear = current_player_control.gear if current_player_control else self._control.gear # Sync gear
                        world.hud.notification('%s Transmission' %
                                            ('Manual' if self._control.manual_gear_shift else 'Automatic'))
                    elif self._control.manual_gear_shift and event.key == K_COMMA: # Gear down
                        self._control.gear = max(-1, self._control.gear - 1)
                    elif self._control.manual_gear_shift and event.key == K_PERIOD: # Gear up
                ...
                # Ensure joystick exists before trying to access event.joy
                if self._steer_joystick is None: continue

                if event.joy == self._steer_joystick_idx: # Check if the event is from the primary joystick
                    if event.button == 13:  # Example: PS button or equivalent for restart
                        world.restart()

                if isinstance(self._control, carla.VehicleControl) and world.player:
                    # Handbrake
                    if event.joy == self._steer_joystick_idx and event.button == self._handbrake_button_idx:
                        self._control.hand_brake = not self._control.hand_brake # Toggle handbrake
                    ...
                    if event.joy == self._steer_joystick_idx and event.button == self._reverse_button_idx:
                        ...
                        if self._control.gear == -1: # If currently in reverse
                            self._control.gear = 1 if player_velocity > 0.1 else 0 # Shift to Drive or Neutral
                            self._control.reverse = False
                            world.hud.notification('Gear: D' if self._control.gear == 1 else 'Gear: N')
                        else: # If in Drive or Neutral
                            self._control.gear = -1 # Shift to Reverse
                            self._control.reverse = True
                            world.hud.notification('Gear: R')

                    # Manual/Automatic Transmission Toggle
                    ...
                    if self._control.manual_gear_shift and event.joy == self._steer_joystick_idx and event.button == self._gear_up_button_idx:
                        self._control.gear = self._control.gear + 1 # No upper limit here, CARLA handles max gear
                    ...
                    if self._control.manual_gear_shift and event.joy == self._steer_joystick_idx and event.button == self._gear_down_button_idx:
                        self._control.gear = max(-1, self._control.gear - 1) # Min gear is -1 (Reverse)
        ...
        if not self._autopilot_enabled and world.player:
            if isinstance(self._control, carla.VehicleControl):
                if self._steer_joystick: # Only parse wheel if joystick is available
                    self._parse_vehicle_wheel()
                else: # Fallback to keyboard if no joystick
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        steer_increment = 5e-4 * milliseconds # Smoother steering
        if keys[K_LEFT] or keys[K_a]:
            self._steer_cache -= steer_increment
        elif keys[K_RIGHT] or keys[K_d]:
            self._steer_cache += steer_increment
        else:
            self._steer_cache = 0.0 # Reset steer if no key is pressed
        self._steer_cache = min(0.7, max(-0.7, self._steer_cache)) # Clamp steer
    ...
    def _parse_vehicle_wheel(self):
        # Ensure joysticks are initialized before trying to get axis values
        if not self._steer_joystick or not self._pedal_joystick:
            # Default to no input if joysticks are not available
        ...
        try:
            ...
            # Throttle and Brake often range from -1 (released) to 1 (fully pressed)
            # Or 0 (released) to 1 (fully pressed) for some pedals.
            # The common configuration for triggers is -1 (released) to 1 (pressed)
            # We need to normalize this to 0 (released) to 1 (pressed)
            raw_throttle = self._pedal_joystick.get_axis(self._throttle_axis_idx) # e.g., R2 trigger
            raw_brake = self._pedal_joystick.get_axis(self._brake_axis_idx)       # e.g., L2 trigger

        except pygame.error as e:
            print(f"Error reading joystick axis: {e}")
            # Fallback to neutral if there's an error reading axes
            raw_steer, raw_throttle, raw_brake = 0.0, -1.0, -1.0 # Assuming -1 is released for pedals

        # Steering: Apply deadzone and linearity
        steer_cmd = raw_steer
        if abs(steer_cmd) < self._steer_deadzone:
            steer_cmd = 0.0
        else:
            # Apply linearity: steer_cmd = sign(raw_steer) * (|raw_steer|^linearity_factor)
            # A simple way to add non-linearity (sensitivity curve)
            # For linearity < 1, less sensitive around center. For > 1, more sensitive.
            # Example: steer_cmd = math.copysign(pow(abs(raw_steer), self._steer_linearity), raw_steer)
            # A more common approach is to scale it after deadzone:
            sign = 1 if steer_cmd > 0 else -1
            steer_cmd = sign * ((abs(steer_cmd) - self._steer_deadzone) / (1.0 - self._steer_deadzone))
            steer_cmd = math.copysign(pow(abs(steer_cmd), self._steer_linearity if self._steer_linearity > 0 else 1.0), steer_cmd)


        # Throttle: Normalize from [-1, 1] or [0, 1] to [0, 1]
        # If your throttle axis is -1 (released) to 1 (pressed):
        throttle_cmd = (raw_throttle + 1.0) / 2.0
        # If your throttle axis is 0 (released) to 1 (pressed) or 0 to -1 (pressed):
        # Check your specific controller's output. For Fanatec, it might be inverted.
        # throttle_cmd = 1.0 - ((raw_throttle + 1.0) / 2.0) # Example for inverted axis

        # Brake: Normalize from [-1, 1] or [0, 1] to [0, 1]
        # If your brake axis is -1 (released) to 1 (pressed):
        brake_cmd = (raw_brake + 1.0) / 2.0
        # If your brake axis is 0 (released) to 1 (pressed) or 0 to -1 (pressed):
        # brake_cmd = 1.0 - ((raw_brake + 1.0) / 2.0) # Example for inverted axis

        # Apply deadzone to pedals
        if throttle_cmd < self._pedal_deadzone: throttle_cmd = 0.0
        if brake_cmd < self._pedal_deadzone: brake_cmd = 0.0
        
        # Clamp values to ensure they are within [0, 1] for throttle/brake and [-1, 1] for steer
    ...
    def _parse_walker_keys(self, keys, milliseconds):
        ...
        if keys[K_DOWN] or keys[K_s]: self._control.speed = 0.0 # Stop
        if keys[K_LEFT] or keys[K_a]:
            self._control.speed = .01 # Minimal speed for turning animation
            self._rotation.yaw -= 0.08 * milliseconds
        if keys[K_RIGHT] or keys[K_d]:
            self._control.speed = .01 # Minimal speed for turning animation
            self._rotation.yaw += 0.08 * milliseconds
        if keys[K_UP] or keys[K_w]:
            self._control.speed = 5.556 if pygame.key.get_mods() & KMOD_SHIFT else 2.778 # Run or walk
...
class HUD(object):
    def __init__(self, width, height):
        ...
        secondary_font_size = 20 # Keep this as the size for other secondary labels
        score_font_size = 36    
        ...
        try:
            ...
            sound_file_path = 'alert_sound.wav' 
            if os.path.exists(sound_file_path):
                self.alert_sound = pygame.mixer.Sound(sound_file_path)
                self.alert_sound.set_volume(0.5)
            else:
                print(f"Warning: Alert sound file not found at '{sound_file_path}'. Sound disabled.")
                self.alert_sound = None
        except pygame.error as e:
            print(f"Warning: Could not initialize pygame.mixer or load alert sound: {e}. Sound disabled.")
            self.alert_sound = None

        # MODIFIED: Sound Cooldowns and Timestamps
        self.LANE_VIOLATION_SOUND_COOLDOWN = 3.0  # seconds
        self.SPEEDING_SOUND_COOLDOWN = 5.0  # seconds
        self._last_sound_time_lane_violation = 0.0
        self._last_sound_time_speeding = 0.0
        # Collision sounds play per penalty, which has its own cooldown in CollisionSensor
        ...
        self._notification_base_pos_y = self.dim[1] - 50 
        self._notification_spacing = 5 
    ...
    def reset_warning_trackers(self):
        ...
        # MODIFIED: Reset sound timestamps
        self._last_sound_time_lane_violation = 0.0
        self._last_sound_time_speeding = 0.0
    ...
    def deduct_score(self, points, violation_type="unknown"):
        ...
        penalty_message = ""
        play_sound_this_time = False
        current_time = time.time() # Get current time for sound cooldown checks

        if violation_type == "collision":
            self.total_points_lost_collisions += points
            penalty_message = f"COLLISION! -{points} PTS"
            # Collision sound plays if a penalty is applied (penalty cooldown is in CollisionSensor)
            play_sound_this_time = True 
            ...
            # MODIFIED: Check sound cooldown for lane violation
            if current_time > self._last_sound_time_lane_violation + self.LANE_VIOLATION_SOUND_COOLDOWN:
                play_sound_this_time = True
                self._last_sound_time_lane_violation = current_time
        else: 
            penalty_message = f"{violation_type.replace('_', ' ').upper()}! -{points} PTS"
            play_sound_this_time = True # Default to play sound for other types

        self.notification(penalty_message, 
                          seconds=2.5, text_color=(255,50,50), 
                          symbol_enabled=True, symbol_color=(255,100,100), 
                          is_blinking=False, is_critical_center=False)

        # MODIFIED: Play sound based on the flag and if sound is available
        if play_sound_this_time and self.alert_sound:
    ...
    def tick(self, world, clock):
        ...
        if speed_kmh > 120: 
            if self.frame > self._last_speed_warning_frame_warned + 180: 
                ...
                # MODIFIED: Check sound cooldown for speeding
                current_time = time.time()
                if current_time > self._last_sound_time_speeding + self.SPEEDING_SOUND_COOLDOWN:
                    if self.alert_sound: self.alert_sound.play()
                    self._last_sound_time_speeding = current_time
    ...
    def critical_alert(self, text, seconds=2.0, text_color=(255, 255, 255), symbol_color=(255, 255, 0)):
        self.notification(text, seconds=seconds, text_color=text_color, 
                          ...
                          is_blinking=True, is_critical_center=True)
        # Critical alerts (like errors or major game events) should probably always play sound if available
        # This is separate from violation-specific cooldowns.
        if self.alert_sound: self.alert_sound.play()
    ...
    def notification(self, text, seconds=2.0, text_color=(255,255,255), 
        ...
        font_size = 30 if is_critical_center else 22 
        symbol_size = 36 if is_critical_center else 28
        ...
        notif_width = self.dim[0] * 0.6 if is_critical_center else 350 
        notif_height = 60 if is_critical_center else 40 
    ...
    def error(self, text):
        self.notification('ERROR: %s' % text.upper(), seconds=5.0, text_color=(255,50,50), 
                          symbol_enabled=True, symbol_color=(255,0,0), 
                          is_blinking=True, is_critical_center=True)
        if self.alert_sound: self.alert_sound.play() # Error sounds always play
    ...
    def render(self, display):
        ...
        for notif_obj in reversed(self._active_notifications): 
            ...
            if notif_obj.is_critical_center:
                ...
                if notif_y < self.dim[1] * 0.4: break 
...
class BlinkingAlert(object):
    ...
    def set_text(self, text, text_color=(255, 255, 255), seconds=2.0, 
        ...
        display_text = text.upper() if self.is_critical_center or self.is_blinking else text
        text_texture = self.font.render(display_text, True, text_color)

        padding = 15 if self.is_critical_center else 10
        content_height = max(symbol_texture.get_height() if symbol_texture else 0, text_texture.get_height())
        box_height = content_height + 2 * padding

        content_width = text_texture.get_width()
        if symbol_texture:
            content_width += symbol_texture.get_width() + (padding // 2) 
        ...
        background_color = (0,0,0,200) if self.is_critical_center else (0,0,0,170)
        border_radius = 10 if self.is_critical_center else 5
        pygame.draw.rect(self.surface, background_color, self.surface.get_rect(), border_radius=border_radius)

        blit_x_start = padding
        current_blit_x = blit_x_start

        symbol_y_offset = (box_height - (symbol_texture.get_height() if symbol_texture else 0)) // 2
        text_y_offset = (box_height - text_texture.get_height()) // 2

        if symbol_texture:
            self.surface.blit(symbol_texture, (current_blit_x, symbol_y_offset))
            current_blit_x += symbol_texture.get_width() + (padding // 2)
        
        self.surface.blit(text_texture, (current_blit_x, text_y_offset))

        if self.is_critical_center:
            self.initial_pos = [(self.screen_dim[0] - box_width) // 2, (self.screen_dim[1] - box_height) // 2]
        else:
...
class LaneInvasionSensor(object):
    ...
    def _on_invasion(weak_self, event):
        ...
        if current_frame > self._last_penalty_frame + 30 : 
            self.hud.deduct_score(LANE_VIOLATION_PENALTY, "lane_violation")
...
class CameraManager(object):
    ...
    def set_sensor(self, index, notify=True):
        ...
        if self.sensors[index][-1] is None:
            logging.warning(f"Cannot set sensor: Blueprint for {self.sensors[index][0]} is missing.")
...
def game_loop(args):
    ...
    try:
        ...
        hud = HUD(native_width, native_height) 
