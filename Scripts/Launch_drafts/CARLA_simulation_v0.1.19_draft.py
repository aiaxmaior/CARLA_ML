...
class HUD(object):
    def __init__(self, width, height):
        ...
        self.LANE_VIOLATION_SOUND_COOLDOWN = 3.0  
        self.SPEEDING_SOUND_COOLDOWN = 5.0  
        ...
        # MODIFIED: Stacked notifications start higher up
        self._notification_base_pos_y = int(self.dim[1] * 0.75) # Start stacking from 75% down
    ...
    def notification(self, text, seconds=2.0, text_color=(255,255,255), 
        ...
        # MODIFIED: Enlarged font sizes
        font_size = 40 if is_critical_center else 30 
        symbol_size = 48 if is_critical_center else 36

        # Font is already fetched as bold by _find_font if available
        ...
        notif_height = 60 if is_critical_center else 40 # Base height, will be adjusted by BlinkingAlert
    ...
    def render(self, display):
        ...
        for notif_obj in reversed(self._active_notifications): 
            ...
            if notif_obj.is_critical_center:
                ...
                # MODIFIED: Allow stacked notifications to go higher
                if notif_y < self.dim[1] * 0.25: break # Stop if stack goes above 25% from top
...
class BlinkingAlert(object):
    def __init__(self, font, screen_dim, initial_dim, symbol_font=None):
        ...
        # MODIFIED: Outline properties
        self.outline_color = (0, 0, 0) # Black outline
        self.outline_thickness = 1 # 1 pixel outline for simple effect, can be 2 for thicker
    ...
    def set_text(self, text, text_color=(255, 255, 255), seconds=2.0, 
        ...
        # Calculate height based on main texture, outline is within this
        content_main_height = max(symbol_texture_main.get_height() if symbol_texture_main else 0, text_texture_main.get_height())
        box_height = content_main_height + 2 * padding

        content_main_width = text_texture_main.get_width()
        if symbol_texture_main:
            content_main_width += symbol_texture_main.get_width() + (padding // 2) 
        box_width = content_main_width + 2 * padding
        ...
        current_blit_x_main = blit_x_start
        ...
        for dx in range(-self.outline_thickness, self.outline_thickness + 1):
            for dy in range(-self.outline_thickness, self.outline_thickness + 1):
                if dx == 0 and dy == 0: continue # Don't draw outline at the exact same spot as main text
                # Simple 4-directional outline, or 8-directional if thickness > 1 and you want diagonals
                if abs(dx) + abs(dy) <= self.outline_thickness : # For 1px, this is up, down, left, right
                     offsets.append((dx, dy))
        if not offsets and self.outline_thickness > 0: # Ensure at least 4-dir for 1px
            offsets = [(-1,0), (1,0), (0,-1), (0,1)]


        if symbol_texture_outline:
            current_blit_x_outline_pass = blit_x_start
            for dx, dy in offsets:
                self.surface.blit(symbol_texture_outline, (current_blit_x_outline_pass + dx, symbol_y_main + dy))
            current_blit_x_outline_pass += symbol_texture_outline.get_width() + (padding // 2)
            
            for dx, dy in offsets:
                 self.surface.blit(text_texture_outline, (current_blit_x_outline_pass + dx, text_y_main + dy))
        else: # Only text outline
            for dx, dy in offsets:
                self.surface.blit(text_texture_outline, (blit_x_start + dx, text_y_main + dy))


        # --- Render Main Text Layer (on top of outline) ---
        if symbol_texture_main:
            self.surface.blit(symbol_texture_main, (current_blit_x_main, symbol_y_main))
            current_blit_x_main += symbol_texture_main.get_width() + (padding // 2)
        
        self.surface.blit(text_texture_main, (current_blit_x_main, text_y_main))


        # MODIFIED: Position critical notifications higher
        if self.is_critical_center:
            # Position its center around 40% down the screen
