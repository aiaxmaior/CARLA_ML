...
Versioning: v0.1.32 (Fixed LaneMarkingType and HUD Notification Call)
...
class HUD(object):
    def __init__(self, width, height, args): # Added args to __init__
        ...
        try:
            if os.path.exists(self.custom_notification_font_path):
                ...
                pygame.font.Font(self.custom_notification_font_path, 1) # Test load
    ...
    def on_world_tick(self, timestamp):
        ...
        self.server_fps = self._server_fps
...
class LaneInvasionSensor(object):
    ...
    def _on_invasion(weak_self, event):
        ...
        logging.debug(f"Lane invasion detected: {event.crossed_lane_markings}") # DEBUG LOG
        ...
        if not occupied_waypoint:
            ...
            if current_frame > self._last_penalty_frame + 30:
                ...
                logging.debug("Lane invasion: No occupied waypoint found, triggering unknown violation.") # DEBUG LOG
            return
        ...
        # Fixed: Use .length() and square it, as Vector2D does not have length_squared()
        if player_forward_vec_2d.length()**2 > 0: 
        ...
        if dot_product_with_occupied_lane < -0.7: # Threshold for "oncoming"
            ...
            logging.debug(f"Lane invasion: Oncoming violation detected (dot product: {dot_product_with_occupied_lane}).") # DEBUG LOG
        else:
            # Check for solid line crossings if not an oncoming violation
            for marking in event.crossed_lane_markings:
                # Corrected: Use carla.LaneMarkingType.Solid and carla.LaneMarkingType.SolidSolid
                # Removed non-existent LaneMarkingType attributes like DoubleSolidYellow, SolidYellow, SolidWhite
                if marking.type in [carla.LaneMarkingType.Solid, 
                    ...
                    logging.debug(f"Lane invasion: Solid line crossing detected (marking type: {marking.type}).") # DEBUG LOG
                    break 
        ...
        if current_frame > self._last_penalty_frame + 30:  # 30 frames cooldown
            ...
            self._last_penalty_frame = current_frame
            logging.debug(f"Lane invasion penalty applied: {violation_type_for_penalty}, amount: {actual_penalty_amount}") # DEBUG LOG
